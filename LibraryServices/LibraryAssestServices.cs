...
{
    public class LibraryAssestServices : ILibraryAssest
    {
        private LibraryContext _context;
        public LibraryAssestServices(LibraryContext context)
        {
            _context = context;
        ...
        {
            var book = _context.LibraryAssets.OfType<Books>().Where(b => b.Id == id);

            return book.Any() ? "Books" : "Video";
        }
        public string GetAuthorOrDirector(int id)
        {
            var isBook = _context.LibraryAssets.OfType<Books>().Where(asset => asset.Id == id).Any();
