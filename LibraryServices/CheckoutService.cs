...
namespace LibraryServices
{
    public class CheckoutService : ICheckout
    {
        ...
        public void CheckInItem(int assetId)
        {
            ...
            //Close and existing checkout history

            CloseExistingCheckoutHistory(assetId, now);

            //look for existing holds on the item
            var currentHolds = _context.Holds.Include(h => h.LibraryAsset).Include(h => h.LibraryCard).Where(h => h.LibraryAsset.Id == assetId);

            //LibraryCard with earliest hold
            if (currentHolds.Any())
            {
        ...
        public IEnumerable<Checkout1> GetAll()
        {
            return _context.Checkouts;
        }

        public Checkout1 GetById(int checkoutId)
        {
        ...
        {
            var checkout = GetCheckedoutByAssetId(assetId);
            if (checkout == null)
            {
                return "";
            };
        ...
        private Checkout1 GetCheckedoutByAssetId(int assetId)
        {
            return _context.Checkouts.Include(h => h.LibraryAsset).Include(h => h.LibraryCard).FirstOrDefault(h => h.Id == assetId);
        }
        ...
        public string GetCurrentHoldPatronName(int holdId)
        {
            ...
            var parton = _context.Patrons.Include(p => p.LibraryCard).FirstOrDefault(p => p.LibraryCard.Id == cardId);
        ...
        public Checkout1 GetLatestCheckout(int assetId)
        {
            return _context.Checkouts.Where(c => c.LibraryAsset.Id == assetId).OrderByDescending(c => c.Since).FirstOrDefault();
        }
